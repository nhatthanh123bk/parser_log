...
class ParserLogs() extends StepsParseLog {
  override def readFileLog(spark: SparkSession, path: String): sql.DataFrame = {
    var logsDf = spark.read.json(path)
    return logsDf
  }
  override def reDefiniteTypeColumn(df: sql.DataFrame, infoAllColumn: InfoAllColumn): sql.DataFrame = {
    var logsDf = df
  ...
  override def parseRequestUriColumn(df: sql.DataFrame, infoRequestUri: InfoRequestUriColumn): sql.DataFrame = {
    var logsDf = df
